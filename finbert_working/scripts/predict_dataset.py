...
output = "predictions_apple_test.csv"

def store(dict, date, platform, score_title, score_summary):
    dict["date"].append(date)
    # dict["platform"].append(platform)
...
df.sort_values(by=['date', 'platform'])
last_date, last_platform = df["date"][0], df["platform"][0]
...
for i in range(length):
    ...
    if ((last_date) != (date)):
        store(output_dict, last_date, last_platform, cul_score_title/counter, cul_score_summary/counter)
        # initialize again
        cul_score_title, cul_score_summary, counter = 0, 0, 0
    ...
    counter += 1
    last_date, last_platform = date, platform
